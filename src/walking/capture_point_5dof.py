"""
Capture Point Walking Controller for Bipedal Robot (5-DOF Legs).
...
K_PT = -15        # Orientation control gain (tuned for 5DOF)
...
# Actuator indices (5-DOF per leg)
CTRL_IDX = {
...
def turn_controller(q_l, q_r, dz_omega, t, stance_leg):
    """
    Control hip yaw joints for turning.
    
    Args:
        q_l: Left hip yaw joint position
        q_r: Right hip yaw joint position
        dz_omega: Turn rate (rad/s)
        t: Time since stance switch
        stance_leg: "Left" or "Right"
    
    Returns:
        dq_stance: Stance leg yaw velocity
        dq_swing: Swing leg yaw velocity
    """
    ...
    if stance_leg == "Right":
        if delta_omega > 0:
            offset = min(delta_omega / 2, q_r)
        else:
            offset = max(delta_omega / 2, q_r)
        ...
        if delta_omega > 0:
            offset = min(delta_omega / 2, q_l)
        else:
            offset = max(delta_omega / 2, q_l)
...
class WalkingController:
    """Capture point based walking controller for 5-DOF leg robot."""
    ...
    def step(self, contact_l, contact_r, q, dx_des, dy_des, dz_omega, t):
        ...
        Args:
            ...
            q: Joint positions (10 elements for 5-DOF legs)
            dx_des: Desired forward velocity
            dy_des: Desired lateral velocity
            dz_omega: Desired turn rate
            t: Current time
        ...
        # Compute turn velocities
        dqz_stance, dqz_swing = turn_controller(
            q[0], q[5], dz_omega,
            t - self.switch_props["t0"],
            self.stance_side
        )
        ...
        # Stance leg: position + yaw control
        v_stance = R_t.T @ vel_stance.reshape(3, 1)
        vel_vec_stance = np.array([
            [v_stance[0, 0]], [v_stance[1, 0]], [v_stance[2, 0]], [5 * dqz_stance]
        ])
        dq_stance = np.linalg.pinv(J_stance) @ vel_vec_stance
        
        # Swing leg: position + yaw control
        v_swing = R_t.T @ vel_swing.reshape(3, 1)
        vel_vec_swing = np.array([
            [v_swing[0, 0]], [v_swing[1, 0]], [v_swing[2, 0]], [5 * dqz_swing]
        ])
        ...
        if self.stance_side == "Right":
            return {
                "right_hip_z": dq_stance[0, 0],
                "right_hip_y": dq_stance[1, 0],
                "right_hip": dq_stance[2, 0],
                "right_knee": dq_stance[3, 0],
                "left_hip_z": dq_swing[0, 0],
                "left_hip_y": dq_swing[1, 0],
                "left_hip": dq_swing[2, 0],
                "left_knee": dq_swing[3, 0]
            }
        else:
            return {
                "left_hip_z": dq_stance[0, 0],
                "left_hip_y": dq_stance[1, 0],
                "left_hip": dq_stance[2, 0],
                "left_knee": dq_stance[3, 0],
                "right_hip_z": dq_swing[0, 0],
                "right_hip_y": dq_swing[1, 0],
                "right_hip": dq_swing[2, 0],
                "right_knee": dq_swing[3, 0]
            }
...
def main():
    ...
    xml_path = os.path.join(
        ...
        "../../models/xml/biped_3d_5dof_leg.xml"
    )
    ...
    controller = KeyboardController(v_step=0.45, v_side=0.15, yaw_step=1.0)
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running():
            ...
            # Get joint angles (10 joints for 5-DOF legs)
            q = np.array([
            ...
            if int(t / dt) % 30 == 0:
                dx, dy, dz_omega = controller.get_cmd()
